...
export default function ResultsPanel({ festival, plantilla, recalcTrigger }) {
  ...
  const handleGenerateResults = async () => {
    ...
    try {
      ...
      // --- ACTUALIZAR GANADOR EN FIRESTORE ---
      if (finalScoresGenerales.length > 0) {
        // Ordenar por puntaje descendente
        const sortedScores = [...finalScoresGenerales].sort((a, b) => b.puntaje - a.puntaje);
        const winner = sortedScores[0];

        // Guardar en el documento del festival
        const festivalRef = doc(db, 'festivales', festival.id);
